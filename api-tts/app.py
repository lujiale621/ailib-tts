...
logger = logging.getLogger("vits-api")
...
file_names = ['VITS', 'HUBERT-VITS', 'W2V2-VITS']
for file_name in file_names:
    with open("roles/"+file_name, 'w', encoding='utf-8') as f:
        for speaker in speakers[file_name]:
            f.write(f"{speaker['id']} {speaker['lang']} {speaker['name']}\n")
...
def voice_vits_api():
    try:
        if request.method == "GET":
            ...
            length = float(request.args.get("length", app.config.get("LENGTH", 1.2)))
            ...
            length = float(request.form.get("length", app.config.get("LENGTH", 1.2)))
